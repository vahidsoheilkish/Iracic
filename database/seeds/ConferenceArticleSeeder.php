...
class ConferenceArticleSeeder extends Seeder
{
    /**
    ...
    private function getDirName($group,$major,$conference_id,$volume_id){
        ...
        $dir = $group."/".$major."/".$conference_id."/".$volume_id."/".$timestamp;
        return $dir;
    }
    ...
    public function run(Faker\Generator $faker)
    {
        for($i=1 ; $i<=100; $i++){
            ...
            $conference_id = $faker->numberBetween(1,299);
            $volume_id = $faker->numberBetween(1,299);
            $dir = $this->getDirName($group_id,$major_id,$conference_id,$volume_id);
            $pre_IOI = explode('/', $dir);
            $IOI = implode('-',$pre_IOI);
            $author = array();
            ...
            try{
                \App\ConferenceArticle::create([
                    'volume_id' => \App\ConferenceVolume::all()->random()->id,
                ...
                echo "Error conference article in {$i} ...\n";
                continue;
            }
        }
    }
