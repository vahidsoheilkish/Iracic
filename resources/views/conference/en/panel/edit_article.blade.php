...
@section('styles')
    <style>
        ...
        #tbl_author label{font-size:11px;}
    </style>
@endsection
...
@section('content')
    <div class="row">
        ...
        <h6 class="alert alert-primary tac">Edit: {{ $article->title }}</h6>
            ...
            <form action="{{ route('conference.notice.article.update',['article'=>$article]) }}" method="post" enctype="multipart/form-data">
                {{ method_field('PATCH') }}
                @csrf
                        ...
                        @foreach($authors as $author)
                            <div class="authors_form" id="authors_form_{{ $loop->index+1 }}">
                                ...
                                <div class="one_author col-sm-5" style="display:inline-table;">
                                    <table id="tbl_author" class="table table-borderless">
                                        <tr>
                    ...
                    <table id="tbl_upload" class="table table-bordered">
                        ...
                        @foreach($files as $file)
                            <table id="tbl_upload" class="table table-responsive-sm">
                                ...
                                <tr>
                                    <td>{{ $file }}</td>
                                    <td><a href="{{conference_article_path}}{{$article->files_directory}}/{{$file}}" target="_blank"><span class="fa fa-file-pdf-o" style="margin:0 4px;"></span>Download</a></td>
                                    <td><button class="btn btn-sm btn-danger" type="button" onclick="deleteArticlePdf('{{conference_article_path}}{{$article->files_directory}}/{{$file}}')"><span class="fa fa-remove" style="margin:0 4px;"></span>Delete article pdf</button></td>
                                </tr>
                            </table>
                        @endforeach
                    </table>
    ...
    <input type="hidden" id="conference_lang" value="{{ $article->conference->lang}}"/>
        ...
        function deleteArticlePdf(path){
            swal({
                ...
                .then((value) => {
                    if(value=="delete"){
                        $.ajax({
                            'url' : '{{route("conference.notice.article.delete.pdf")}}',
                            'type' : 'post',
