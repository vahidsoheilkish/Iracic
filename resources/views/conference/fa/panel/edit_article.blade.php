...
@section('content')
    <div class="row tar">
        ...
        <h6 class="alert alert-primary tac">Edit: {{ $article->title }}</h6>
            ...
            <form action="{{ route('conference.notice.article.update.fa',['article'=>$article]) }}" method="post" class="rtl" enctype="multipart/form-data">
                {{ method_field('PATCH') }}
                @csrf
                ...
                <div class="row form-group">
                    <div class="col-sm-6">
                        ...
                        <input type="text" name="accepted_date" id="accepted_date" class="form-control" placeholder="تاریخ پذیریش" readonly required value="{{ $article->accepted['date'] }}"/>
                    </div>
                    <div class="col-sm-6">
                        <label for="receieved_date" style="margin:6px;"><span class="star">*</span>تاریخ دریافت</label>
                        <input type="text" name="receieved_date" id="receieved_date" class="form-control" placeholder="تاریخ دریافت" readonly required value="{{ $article->receieved['date'] }}"/>
                    </div>
                </div>
                <div class="form-group">
    ...
    <input type="hidden" id="conference_lang" value="{{ $article->lang}}"/>
        ...
        function deleteArticlePdf(path){
            swal({
                ...
                .then((value) => {
                    if(value=="delete"){
                        $.ajax({
                            'url' : '{{route("conference.notice.article.delete.pdf.fa")}}',
                            'type' : 'post',
