...
@section('content')
    <div class="row">
        <div class="col-sm-12" style="margin-top:50px;">
            <h6 class="alert alert-primary tac">Edit: {{ $article->title }}</h6>
            ...
            <form action="{{ route('user.publication.article.update',['article'=>$article]) }}" method="post" enctype="multipart/form-data">
                {{ method_field('PATCH') }}
                @csrf
                <div class="form-group">
                    <label for="price" style="margin:6px;"><span class="star">*</span>Article Price</label>
                ...
                <div class="form-group">
                    <label for="title" style="margin:6px;"><span class="star">*</span>Title</label>
                ...
                <div class="col-xl-12 col-lg-12 col-md-12 col-sm-12 col-xs-12 tal" style="margin-bottom: 20px;">
                    <button onclick="addAuthor()" type="button" id="add_author_btn" class="btn btn-warning btn-sm tal" style=""><i class="fa fa-plus" style="vertical-align:-2px; margin:0 4px;"></i>New author</button>
                    <p class="tar" style="float:right; background-color: #a42e2e; color:#fff; border-radius: 4px; font-size:16px; padding:10px; min-width:50px;">Specifications of authors article</p>
                </div><hr/>
                @if($authors)
                    <div class="form-inline form-vertical" style="float:right;">
                        @foreach($authors as $author)
                            <div class="authors_form" id="authors_form_{{ $loop->index+1 }}">
                                <p class="tar author_list" style="">Author
                                    {{  $loop->index+1  }}
                                </p>
                                <div class="one_author col-sm-5" style="display:inline-table;">
                                    <table class="table table-borderless">
                                        <tr>
                                            <td><label for="author_name_{{$loop->index+1}}"><span class="star">*</span>Author name</label></td>
                                            <td><input type="text" name="author_name[]" id="author_name_{{$loop->index+1}}" class="form-control-sm" value="{{ $author->name }}" required /></td>
                                        </tr>

                                        <tr>
                                            <td><label for="author_email_{{$loop->index+1}}"><span class="star">*</span>Author email</label></td>
                                            <td><input type="text" name="author_email[]" id="author_email_{{$loop->index+1}}" class="form-control-sm" value="{{ $author->email }}" required /></td>
                                        </tr>

                                        <tr>
                                            <td><label for="author_rate_{{$loop->index+1}}"><span class="star">*</span>Author education</label></td>
                                            <td><input type="text" name="author_rate[]" id="author_rate_{{$loop->index+1}}" class="form-control-sm" value="{{ $author->rate }}" required /></td>
                                        </tr>

                                        <tr>
                                            <td><label for="author_dependency_{{$loop->index+1}}"><span class="star">*</span>Author dependence</label></td>
                ...
                <div class="row form-group rtl">
                    ...
                    <div class="col-sm-4">
                        <label for="page_count" style="margin:6px;"><span class="star">*</span>Page count</label>
                    ...
                    <div class="col-sm-4">
                        ...
                        <input type="number" min="1" max="1000" name="page_from" id="page_from" class="form-control label-success" placeholder="" required value="{{ $page_start_end[0] }}"/>
                    </div>
                    <div class="col-sm-4">
                        <label for="page_to" style="margin:6px;"><span class="star">*</span>End page</label>
                        <input type="number" min="1" max="1000" name="page_to" id="page_to"  class="form-control label-warning" placeholder="" required value="{{ $page_start_end[1] }}"/>
                    </div>
                </div><hr/>
                <div class="row form-group">
                    <div class="col-sm-3">
                ...
                <div class="form-group">
                    @foreach($structs as $structKey => $struct)
                        <span class="btn btn-info" style="position: relative; bottom:25px;">Structures
                            {{ $loop->index+1  }}
                        </span>
                        <div class="row">
                            <div class="col-sm-6" style="border-left:2px solid #222;">
                                <table class="table table-borderless">
                                    <tr>
                                        <td><label class="line_height_40 font_13" for="struct_title_{{ $loop->index+1 }}">Structure</label></td>
                            ...
                            <div class="col-sm-6" id="">
                                @if( !empty($struct->substr) )
                                    <span style="display:inline-block; padding:4px;" class="alert alert-info">Infrastructure</span>
                ...
                <div class="col-xl-12 col-lg-12 col-md-12 col-sm-12 col-xs-12 tal" style="margin-bottom: 20px; border-bottom:3px dotted #222;">
                    ...
                    <p class="tar" style="float:right; background-color: #4dc2b6; color:#fff; border-radius: 4px;  font-size:16px; padding:10px; min-width:50px;">Articles Structures</p>
                </div>
                <div class="form-group">
                ...
                <div class="form-group tac">
                    <button type="submit" class="btn btn-warning" style="width:50%;">Update article</button>
                </div>
            </form>
        </div>
    </div>
    ...
    <script>
        ...
        function addAuthor(){
            ...
            $("#authors_form_" + nextIndex).append('' +
                '                                <p class="tar author_list" style="background-color:#0d9e4c; color:#ffffff;">Author '+nextIndex+'</p>' +
                '                                <div class="one_author col-sm-5" style="display:inline-table;">\n' +
                '                                    <table class="table table-borderless"> '+
                '                                       <tr> '+
                '                                            <td><label for="author_name_'+nextIndex+'"><span class="star">*</span>Author name</label></td> '+
                '                                            <td><input type="text" name="author_name[]" id="author_name_'+nextIndex+'" class="form-control" value="" required /></td> '+
                '                                            <td><label for="author_email_'+nextIndex+'"><span class="star">*</span>Author email</label></td> '+
                '                                            <td><input type="text" name="author_email[]" id="author_email_'+nextIndex+'" class="form-control ltr" value="" required /></td> '+
                '                                        </tr> '+
                '                                        <tr> '+
                '                                        <td><label for="author_rate_'+nextIndex+'"><span class="star">*</span>Author education</label></td> '+
                '                                        <td><input type="text" name="author_rate[]" id="author_rate_'+nextIndex+'" class="form-control" value="" required /></td> '+
                '                                        <td><label for="author_dependency_'+nextIndex+'"><span class="star">*</span>Author dependence</label></td> '+
                '                                        <td><input type="text" name="author_dependency[]" id="author_dependency_'+nextIndex+'" class="form-control" value="" required /></td> '+
                '                                        </tr> '+
                '                                    </table> '+
                '                           </div>');
        }
        ...
        function AddSubStruct(e){
            ...
            $("#substr_"+div[1]).append('<div class="row tac">' +
                ...
                '<label> Substructure  '+counter+'<label>' +
                '</div>' +
        ...
        function deleteArticlePdf(path){
            swal({
                ...
                .then((value) => {
                    if(value=="delete"){
                        $.ajax({
                            'url' : '{{route("publication.article.delete.pdf")}}',
                            'type' : 'post',
