    ...
    <div class="container-fluid container-publication">
        <div class="container">
            <div class="row text-center">
                <div class="col-12">
                    <h3 id="publication_count" class="text-muted">Showing {{count($publications)}} Journals</h3>
                    ...
                    <ul id="journals_list" class="journals">
                        @foreach($publications as $publication)
                            <li class="journals-section">
                                <a href="{{ route('publication.link',['id'=>$publication->id] ) }}">
                                <span class="section-text1">{{ $publication->title }}</span>
                                </a>
    ...
    <script>
        $(document).ready(function(){
            ...
            $("#group_select").change(function(){
                ...
                $.ajax({
                    ...
                    'success' : function(data){
                        ...
                        history.pushState('data to be passed', 'Title of the page', '/journal/group/'+group+'/major/'+major);
                        $("#group_id").val(group);
                        $("#major_id").val(major);
                    }
                });
                $.ajax({
                    'url' : '/pi/journal/group/' + group +'/major/' + -1 ,
                    'type' : 'post',
                    ...
                    'success' : function(data){
                        ...
                        $(".journals").empty();
                        history.pushState('data to be passed', 'Title of the page', '/journal/group/'+group+'/major/'+ -1);
                        $("#group_id").val(group);
                        $("#major_id").val(-1);
                        $("#pagination").empty();
                        if(data.length > 0){
                            $.each(data , (key,val)=>{
                                $("#journals_list").append('<li class="journals-section">\n' +
                                    '                                <a href="/journal/'+ val._id +' ">\n' +
                                    '                                <span class="section-text1">'+ val.title +'</span>\n' +
                                    '                                </a>\n' +
                                    '                                <div class="section-text2">\n' +
                                    '                                    <span class="text-1 text-muted">journal</span>\n' +
                                    '                                    &diam;\n' +
                            ...
                            $("#journals_list").append('<li class="alert alert-danger" style="text-align:center;">No publication Found</li>');
                        }
                    }
                });
            });
            ...
            $("#major_select").change(function(){
                ...
                $.ajax({
                    'url' : '/pi/journal/group/' + group +'/major/' + major ,
                    'type' : 'post',
                ...
                success : function(result){
                    ...
                    if(result.message="success"){
                        ...
                        if(result.publications.length > 0){
                            $.each(result.publications , (key,val)=>{
                                $("#journals_list").append('<li class="journals-section">\n' +
                                    '                                <a href="/journal/'+ val._id +' ">\n' +
                                    '                                <span class="section-text1">'+ val.title +'</span>\n' +
                                    '                                </a>\n' +
