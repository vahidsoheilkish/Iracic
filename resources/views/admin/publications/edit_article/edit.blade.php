...
@section('content')
    <div class="row tar">
        <div class="col-sm-11" style="margin:auto;">
            ...
            <form action="{{ route('publication.article.update',['article'=>$article]) }}" method="post" class="rtl" enctype="multipart/form-data">
                {{ method_field('PUT') }}
                @csrf
                ...
                <div class="row form-group">
                    <div class="col-sm-6">
                        ...
                        <input type="text" name="accepted_date" id="accepted_date" class="form-control" placeholder="تاریخ پذیریش" readonly required value="{{ date('Y-m-d', strtotime($article['accepted']['date']) ) }}"/>
                    </div>
                    <div class="col-sm-6">
                        <label for="receieved_date" style="margin:6px;"><span class="star">*</span>تاریخ دریافت</label>
                        <input type="text" name="receieved_date" id="receieved_date" class="form-control" placeholder="تاریخ دریافت" readonly required value="{{ date('Y-m-d', strtotime($article['receieved']['date']) ) }}"/>
                    </div>
                </div>
                <div class="form-group">
                ...
                <div class="row form-group rtl">
                    ...
                    <div class="col-sm-4">
                        ...
                        <input type="number" min="1" max="1000" name="page_from" id="page_from" class="form-control-sm label-success" placeholder="" required value="{{ $article->page }}"/>
                    </div>
                    <div class="col-sm-4">
                        <label for="page_to" style="margin:6px;"><span class="star">*</span>صفحه پایان</label>
                        <input type="number" min="1" max="1000" name="page_to" id="page_to"  class="form-control-sm label-warning" placeholder="" required value="{{ $article->page }}"/>
                    </div>
                </div><hr/>
                <div class="row form-group rtl">
                    <div class="col-sm-3">
    ...
    <input type="hidden" id="publication_lang" value="{{ $article->issue->volume->publication->lang}}"/>
        ...
        function deleteArticlePdf(path){
            swal({
                ...
                .then((value) => {
                    if(value=="delete"){
                        $.ajax({
                            'url' : '{{route("admin.publication.article.delete")}}',
                            'type' : 'post',
