...
@section('styles')
    <style>
        ...
        .remove_organizer{float:left;padding:4px; color:#f1f1f1; border-radius:4px; background-color: tomato; margin-top:8px;}
        .remove_organizer:hover{cursor: pointer; transition:.3s; box-shadow:0 0 12px #d2d2d2;}
    </style>
@endsection
                ...
                <div class="page-body">
                    <div class="container" style="margin-bottom:30px; overflow-x:hidden;">
                        ...
                        <div id="conf_form" class="row">
                            <form action="{{ route('admin.conference.store' , ['user'=>$user]) }}" class="rtl" method="post" enctype="multipart/form-data" id="">
                                @csrf
...
@section('scripts')
    <script>
        ...
        function addOrganizer(){
            ...
            $("#organizer_form_" + nextIndex).append('<span class="alert alert-success" style="float: right;"> برگزارکننده '+digitToFarsi(nextIndex)+'</span><span class="fa fa-remove remove_organizer" onclick="removeOrganizer(organizer_form_'+nextIndex+' )"></span>\n' +
                '                            <div class="form-group row mt-3" style="clear: both">\n' +
        ...
        function removeOrganizer(target){
            $(target).remove();
            // let id = $(target).attr('id');
            // id = id.split('_');
            // for(let i=2; i<=id[2]; i++){
            //     $("#organizer_form_"+i).remove();
            // }
        }
