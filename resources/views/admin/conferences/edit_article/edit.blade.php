...
@section('styles')
    <style>
        ...
        .one_author{border-bottom:2px solid #ccc;border-left:1px solid tomato;padding:15px; direction: rtl;}
...
@section('content')
    <div class="row tar">
        <div class="col-sm-11" style="margin: auto;">
            ...
            <form action="{{ route('conference.article.update',['article'=>$article]) }}" method="post" class="rtl" enctype="multipart/form-data">
                {{ method_field('PATCH') }}
                @csrf
                ...
                @if($authors)
                    <div class="form-inline form-vertical rtl" style="float:right;">
                ...
                <div class="row form-group">
                    <div class="col-sm-6">
                        ...
                        <input type="text" name="accepted_date" id="accepted_date" class="form-control" placeholder="تاریخ پذیریش" readonly required value="{{ date('Y-m-d', strtotime($article['accepted']) ) }}"/>
                    </div>
                    <div class="col-sm-6">
                        <label for="receieved_date" style="margin:6px;"><span class="star">*</span>تاریخ دریافت</label>
                        <input type="text" name="receieved_date" id="receieved_date" class="form-control" placeholder="تاریخ دریافت" readonly required value="{{ date('Y-m-d', strtotime($article['receieved']) ) }}"/>
                    </div>
                </div>
                <div class="form-group">
                    ...
                    <table id="tbl_upload" class="table table-bordered">
                        ...
                        @foreach($files as $file)
                            <table id="tbl_upload" class="table table-responsive-sm">
                                ...
                                <tr>
                                    <td style="font-size:11px; overflow: scroll">{{ $file }}</td>
    ...
    <input type="hidden" id="conference_lang" value="{{ $article->volume->conference->lang}}"/>
        ...
        function deleteArticlePdf(path){
            swal({
                ...
                .then((value) => {
                    if(value=="delete"){
                        $.ajax({
                            'url' : '{{route("admin.conference.delete.pdf")}}',
                            'type' : 'post',
